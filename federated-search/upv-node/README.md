...
# Deploymets of Federated Search at UPV-NODE side

Schema of connections: 
```
    Central side (EUCAIM)         |        UPV-NODE side
        beam-broker               
broker.eucaim.cancerimage.eu
            443                 <---   beam-proxy                       dataset-service
                                          8081      <--   focus   -->    11000
```
...
```
This will generate two files: `upv-node.priv.pem` (private key) and `upv-node.csr` (public key).  
You must 
 - copy the contents of `upv-node.priv.pem` to the key `proxy.pem` in the secret `0-secret-certs.private.yaml` (which will be used by the beam-proxy),
 - and send the `upv-node.csr` to the admin of the central broker in order to be our proxy recognized by them.

The admin of broker will have to [sign and store the certificate in the vault](../README.md#sign-and-store-third-party-repo-certificate).
